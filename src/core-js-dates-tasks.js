...
function getNextFriday(date) {
  const dateNextFriday = new Date(date);
  const daysToFriday = (5 - date.getDay() + 7) % 7 || 7;
  dateNextFriday.setDate(date.getDate() + daysToFriday);
  return dateNextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const dateS = Date.parse(dateStart);
  const dateE = Date.parse(dateEnd);
...
function isDateInPeriod(date, period) {
  ...
  if (startDate - ourDate > 0 || endDate - ourDate < 0) return false;
  return true;
}
...
function formatDate(date) {
  const ourDate = new Date(date);
  ourDate.setMinutes(ourDate.getMinutes() + ourDate.getTimezoneOffset());
  const options = {
    month: 'numeric',
    day: 'numeric',
    year: 'numeric',
  ...
  return new Intl.DateTimeFormat('en-US', options).format(ourDate);
}
...
function getWeekNumberByDate(date) {
  ...
  const offsetDays = firstDay.getDay() ? firstDay.getDay() - 1 : 7;

  return Math.ceil((days + offsetDays) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    for (let days = 0; days < countWorkDays; days += 1) {
      ...
      if (startDate >= endDate) return workingDaysArray;
    }
